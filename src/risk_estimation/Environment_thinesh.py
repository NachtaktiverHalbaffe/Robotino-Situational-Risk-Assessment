...
def initialize_map(map_path):
    ...
    corners_hand = [
        tuple(map(lambda i, j: i + j, corners_hand[i], add[i])) for i in range(4)
    ]
    ...
    for o in range(0, len(obstacles)):
        for i in range(0, len(obstacles[o].corners)):
            obstacles_disturbed[o].corners[i] = tuple(
                map(lambda j, k: j + k, obstacles_disturbed[o].corners[i], action_index)
            )
...
def initialize_traj(
    map_ref, obstacles=None, nodes=None, visualize=False, edges_all=None, env=None
):
    """
...
class Environment:
    ...
    def __init__(
        ...
        ) = initialize_traj(self.map_ref, self.obstacles, nodes=None)
    ...
    def modify_map(self):
        ...
        for obstacle in self.obstacles:
            ...
            map_ref_adv_draw.polygon(
                obstacle.corners, fill=(255, 255, 255), outline=(255, 255, 255)
            )
    ...
    def step_adv1(self, action, probability, env):
        ...
        pos_new, angle_new = calc_new_position(
            self.state_adv1.position, self.state_adv1.angle, command_vanilla
        )
        ...
        self.modify_map()
        segment_adv_nodes, segments_adv = calc_adv_traj(
            self.map_ref_adv, segment_adv_coordinates, obstacles_disturbed
        )
        ...
        for i in range(
            0, len(nearest_distances_adv)
        ):  # gives us the nearest distance / point for each segment in a linst
        ...
        for i in range(
            0, len(nearest_distances_vanilla)
        ):  # gives us the nearest distance / point for each segment in a linst
        ...
        if cost_difference >= 9999:  # colission occured
            ...
            for i in range(0, self.state_adv1.traj_index):
                visu_adv_traj_map_draw.line(
                    [
                        (
                            self.trajectory_vanilla[i].coordinates[0],
                            self.trajectory_vanilla[i].coordinates[1],
                        ),
                        (
                            self.trajectory_vanilla[i + 1].coordinates[0],
                            self.trajectory_vanilla[i + 1].coordinates[1],
                        ),
                    ],
                    fill=(255, 255, 0),
                )
            ...
            for i in range(0, self.state_adv1.traj_index - 1):
                visu_adv_traj_map_draw.line(
                    [
                        (
                            self.trajectory_adv[i].coordinates[0],
                            self.trajectory_adv[i].coordinates[1],
                        ),
                        (
                            self.trajectory_adv[i + 1].coordinates[0],
                            self.trajectory_adv[i + 1].coordinates[1],
                        ),
                    ],
                    fill=(0, 0, 255),
                )
        ...
        if reset_traj:
            if not new_nodes:
                self.trajectory_vanilla, self.nodes_vanilla, _ = initialize_traj(
                    self.map_ref, nodes=self.nodes_vanilla, visualize=self.visualize
                )
            else:
                (
                    self.trajectory_vanilla,
