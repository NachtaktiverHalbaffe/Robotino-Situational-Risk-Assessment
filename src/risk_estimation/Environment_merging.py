...
class Environment:
    ...
    def step_adv1(self, action, action_prob):
        ...
        segment_adv_nodes, segments_adv = calc_adv_traj(
            self.map_ref, segment_adv_coordinates, self.obstacles
        )
    ...
    def step_prot(self, action):
        ...
        eps = 1e-12
        traj_length_penalty = 0.5 * (
            1 - length_vanilla / (length_prot + eps)
        )  # this (0.5) is an important hyperparameter that regulates the balance between performance and safety (in combination with the other rewards and cost values...) -> might has to be adjusted!!!
