...
for i, name in enumerate(filenames):
    ...
    ax = (
        (df["rl_prob"] - df["brute_prob"]).abs().plot.hist(bins=division, alpha=0.5, label=f"min len {params[i][1]+1}")
    )

    df_sliced = df.drop(df[(df["rl_prob"] - df["brute_prob"]) <= 0].index)
    ax = (
        (df_sliced["rl_prob"] - df_sliced["brute_prob"])
        .abs()
        .plot.hist(bins=division, alpha=0.5, label="rl is higher")
    )
