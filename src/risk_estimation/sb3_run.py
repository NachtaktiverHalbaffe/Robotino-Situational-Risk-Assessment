...
def test_run(
    configs, plot_dir="throw_away", env_name="robo_navigation-v0", n_envs=4, learning_rate=0.0001, steps=50_000
):
    ...
    env = SubprocVecEnv([lambda: Monitor(gym.make(env_name, config=configs)) for i in range(n_envs)])
...
if __name__ == "__main__":
    ...
    test_run(configs=configs, plot_dir=plot_dir, env_name=env_name, learning_rate=learning_rate, steps=steps)
