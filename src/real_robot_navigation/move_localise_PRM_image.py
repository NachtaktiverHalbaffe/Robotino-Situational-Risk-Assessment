...
def draw_map_location_acml(x, y, rot, draw_frame, base_info, color_FoV=(255, 0, 255), color_outer=(255, 0, 0)):
    ...
    if color_FoV == (255, 0, 255):
        ...
        if write_to_cvs and color_FoV == (0, 0, 255):
            ...
            if rot_shift < 0.20:
                with open(
                    "./real_robot_navigation/error_dist_csvs/loc/error_dist_loc_dist_filtered_PRM.csv", "a"
                ) as f1:
                    write = csv.writer(f1)
                    write.writerow([dist_shift])
        if write_to_cvs and color_FoV == (255, 0, 0):
            ...
            with open("./real_robot_navigation/error_dist_csvs/loc/error_dist_loc_y_PRM_end.csv", "a") as f1:
                ...
                write.writerow([y_shift])
            with open(
                "./real_robot_navigation/error_dist_csvs/loc/error_dist_loc_rot_abs_chair_PRM_end.csv", "a"
            ) as f1:
                write = csv.writer(f1)
                write.writerow([rot_shift])
            ...
            if rot_shift < 0.20:
                with open(
                    "./real_robot_navigation/error_dist_csvs/loc/error_dist_loc_dist_filtered_PRM_end.csv", "a"
                ) as f1:
                    write = csv.writer(f1)
                    write.writerow([dist_shift])
...
def run_detec_and_localise_PRM(weights_detection, weights_localise, use_detection_cam=False, use_localise_cam=True):
    ...
    if use_detection_cam:
        ...
        if detec_movables_obstacles:
            ...
            objects_to_move = []
            map_ref = modify_map(
                map_ref, objects_to_move, detec_movables_obstacles, color=(0, 255, 255), convert_back_to_grey=False
            )
            cv2.imshow("map", np.kron(np.asarray(map_ref.convert("RGB")), np.ones((2, 2, 1))))
            cv2.waitKey(100)

    trajectory_vanilla, nodes_vanilla, edges_all_vanilla = initialize_traj(
        map_ref_PRM, all_obst, nodes=None, start=start, end=[124, 68]
    )
    traj_updt = []
    ...
    for trajectory_node, counter in zip(traj_updt[1:], range(0, len(traj_updt) - 1)):
        ...
        if counter < len(traj_updt) - 1:
            ...
            visu_adv_traj_map_draw.line(
                [
                    (trajectory_vanilla[counter].coordinates[0], trajectory_vanilla[counter].coordinates[1]),
                    (trajectory_vanilla[counter + 1].coordinates[0], trajectory_vanilla[counter + 1].coordinates[1]),
                ],
                fill=(255, 255, 0),
            )
        ...
        if localise_dict:
            ...
            map_ref_loc = modify_map(
                map_ref_loc, [], [obstacles_ws[index_smallest_dist_ws]], color=(255, 0, 0), convert_back_to_grey=False
            )
            map_ref_loc = modify_map(map_ref_loc, [], [detected_obst], color=(0, 255, 0), convert_back_to_grey=False)
            map_ref_loc_draw = ImageDraw.Draw(map_ref_loc)

            # calculating rotation from detected and cord transforms
            # TODO replace with proper pass
