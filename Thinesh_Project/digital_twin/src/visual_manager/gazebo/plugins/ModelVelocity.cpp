...
namespace gazebo
{
  ...
  // Load the controller
  void ModelVelocity::Load(physics::ModelPtr parent,
      sdf::ElementPtr sdf)
  {
    ...
    ros::SubscribeOptions so =
      ros::SubscribeOptions::create<geometry_msgs::Twist>(command_topic_, 1,
          boost::bind(&ModelVelocity::cmdVelCallback, this, _1),
          ros::VoidPtr(), &queue_);

    vel_sub_ = rosnode_->subscribe(command_topic_, 1, &ModelVelocity::cmdVelCallback, this);
    odometry_pub_ = rosnode_->advertise<nav_msgs::Odometry>(odometry_topic_, 1);

    // start custom queue for diff drive
    callback_queue_thread_ =
      boost::thread(boost::bind(&ModelVelocity::QueueThread, this));

    // listen to the update event (broadcast every simulation iteration)
    update_connection_ =
      event::Events::ConnectWorldUpdateBegin(
          boost::bind(&ModelVelocity::UpdateChild, this));

  }

  // Update the controller
  void ModelVelocity::UpdateChild()
  {
    /*
    ...
    std::cout << "x_, y_ ,rot_"<<x_<< y_ <<rot_<<std::endl;
    ...
    std::cout << "before x_, y_ ,rot_"<<x_<< y_ <<rot_<<std::endl;
    x_ = 0;
    if (enable_y_axis_)
    {
      y_ = 0;
    ...
    rot_ = 0;
    std::cout << "after x_, y_ ,rot_"<<x_<< y_ <<rot_<<std::endl;
    */
  }

  // Finalize the controller
  void ModelVelocity::FiniChild() {
    alive_ = false;
    queue_.clear();
    queue_.disable();
    rosnode_->shutdown();
  ...
  void ModelVelocity::cmdVelCallback(
  ...
  void ModelVelocity::publishOdometry(double step_time)
  {
