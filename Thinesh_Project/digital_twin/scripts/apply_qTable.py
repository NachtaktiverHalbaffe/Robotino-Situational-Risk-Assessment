...
def velCallback(velTwist):
    ...
    if Learning and new_pose:
        ...
        if velTwist.linear.x>0.01:
            ...
            if int(round(velTwist.linear.x/resolution,2)) in actionSpace:
                
                qAction = np.argmax(q_table[int(velTwist.linear.x/resolution)])*resolution
            else:
                qAction = np.argmax(q_table[int(np.argmax(actionSpace))])*resolution
                print('maxxxx', (int(velTwist.linear.x/resolution)))
            publishVel(qAction)
