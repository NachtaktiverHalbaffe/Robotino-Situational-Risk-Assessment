...
def plot_rewards_dual(path, log_reader_1,log_reader_2, avg_window=1000):
    ...
    plt.plot(x, rewards_avg_1, label= "Baseline_MAARL", color = 'b')
    plt.plot(x, rewards_avg_2, label = "Probability aware MAARL",color =  'r')
    plt.xlabel('steps')
    plt.ylabel('average reward')
    plt.legend()
    plt.grid()
...
def plot_collisions_dual(path, log_reader1,log_reader2, avg_window=500):
    ...
    for i in range(0, len(steps_1)-1):
        ...
        if steps_1[i+1] <= steps_1[i]:
            collisions_scores_1.append(collision_2)
            collision_1 = 0
        if i == len(steps_1)-2:
    ...
    x = list(range(0, n_windows))
    x = np.array(x)*avg_window
    plt.plot(x, collisions_scores_avg_1,label= "Baseline_MAARL", color = 'b')
    plt.plot(x, collisions_scores_avg_2, label = "Probability aware MAARL",color =  'r')
    plt.xlabel('episode')
    plt.ylabel('average collision rate')
    plt.legend()
    plt.grid()
...
#path, log_reader = CSV_Reader(path = "adv_1", test_mode= False)
#plot_rewards(path, log_reader)
#calc_probabilities(log_reader)
#plot_collisions(path,log_reader)
