...
def convert_grid_to_robo(x_r,y_r):
    ...
    r = 1.204
    r = 1.204
    x_g = x_r*np.cos(r)-y_r*np.sin(r)
    y_g = x_r*np.sin(r)+y_r*np.cos(r)
    return x_g,y_g
...
def get_pixel_location_from_acml(x_a,y_a,x3,y3,x5_3,x5_3_p,y5_3,y5_3_p,rot=None):
    ...
    if not rot == None:
        ...
        x_a = x_a-np.cos(rot)*0.215
        y_a_1 = y_a-np.sin(rot)*0.225
        y_a_2 = y_a-np.sin(rot+0.05)*0.225
        y_a_3 = y_a-np.sin(rot-0.05)*0.225
        changed.update({rot:x_a})
        changed2.update({rot:y_a_2})
        normal2.update({rot:y_a_3})
        lists = sorted(changed.items())
        x,y = zip(*lists)
        plt.plot(x,y)
        # print(changed,changed2,'changed')
        print(sum(changed.values()),sum(changed2.values()),sum(normal2.values()),'0+0.1-0.1')
        plt.ylabel('some numbers')
        plt.show(block=False)
        plt.pause(0.2)
        print(min(normal,key=normal.get),sum(changed.values()))
        print(min(changed,key=changed.get), sum(changed.values()))
...
def get_amcl_from_pixel_location(x_p,y_p,x3,y3,x5_3,x5_3_p,y5_3,y5_3_p,rot=None):
    "get pixel location from acml pos"
    x_convert = x5_3_p/x5_3 #pixel per meter in width, pixels have flipped x axis (is also offset, applied below)
    y_convert = y5_3_p/y5_3 #pixel per meter in hight, pixels have flipped y axis (is also offset, applied below)
    offset_x = -x3*x_convert+100 # calculating x offset based on the pixel location of x3
    ...
    if not rot == None:
        rot = 2*np.arcsin(rot)
        x_a = x_a+np.cos(rot)*0.25
        y_a = y_a+np.sin(rot)*0.25
    return x_a, y_a
...
def distance_to_point(point, local_pos = None):
    global real_data
    if real_data is None:
        local_pos = real_data
    else:
        local_pos = local_pos
    dist_point = (point[0]-local_pos[1], point[1]-local_pos[2])
    rot = -2*np.arcsin(local_pos[3])
    return convert_robo_to_cam(dist_point[0],dist_point[1],rot)

def camera_to_acml_roation(dist, local_pos = None):
    global real_data
    if real_data is None:
        local_pos = real_data
    else:
