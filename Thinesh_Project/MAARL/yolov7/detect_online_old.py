...
from utils.datasets import LoadStreams, LoadImages, LoadImagesDelivered_old
...
def loaded_detect(limg,sources,imgsz,stride,device,half,model,opt,classify,webcam,save_dir,names,save_txt,save_img,view_img,colors,calc_3d,p_matrix,plot_3d):
    t0 = time.time()
    source = sources[0]
    dataset = LoadImagesDelivered_old(source, img_size=imgsz, stride=stride,limg=limg)
    highest_conf = 0
            ...
            if len(det):
                ...
                for *xyxy, conf, cls in reversed(det):
                    ...
                    if save_img or view_img:  # Add bbox to image
                        ...
                        if plot_3d and not boundry and highest_conf< float(label[-3:]):
