...
class RoboEnv_gym_2(gym.Env):
    ...
    def __init__(self, config, env = None):
        ...
        # create Environment
        visualize = True
        self.env_real = Environment(map_path=config['map_path'], relevant_segments=config['relevant_segments'],
                        done_after_collision=config['done_after_collision'], adversary=None,
                        visualize=visualize, start=config['start'], goal=config['goal'])
        ...
        if self.actions_per_dimension==9:
            ...
            self.angle_probs = [0.01395939/2, 0.04060914/2, 0.12817259/2, 0.21700508/2, 0.57614213, 0.21700508/2, 0.12817259/2, 0.04060914/2, 0.01395939/2]
            self.dists = np.array([-9, -7, -5, -3 , 0 , 3, 5, 7, 9])*0.05
            self.dist_probs = [0.10135135/2, 0.17567568/2, 0.30135135/2, 0.23108108/2, 0.06621622, 0.23108108/2, 0.30135135/2, 0.17567568/2, 0.10135135/2]
        else:
    ...
    def reset(self,agent_name = 'adv1', new_nodes=False, start=[62,74], goal=[109,125]):
        ...
        """
        observation, traj_vanilla = self.env_real.reset(agent_name, new_nodes=new_nodes, start=start, goal=goal,persisten_map=self.persisten_map)
    ...
    def step_adv1(self, network_action, action_prob_placeholder = 0):
        ...
        probs_action_sb3 = {'angle_loc':self.angle_probs[action_index_rot],'dist_loc':self.dist_probs[action_index_dist],'combined':self.angle_probs[action_index_rot]*self.dist_probs[action_index_dist]}
        ...
        probs_action_sb3 = {'angle_loc':self.angle_probs[action_index_rot],'dist_loc':1,'combined':self.angle_probs[action_index_rot]*1}
        ...
        obs, reward, done, collision_status, adv1_node2, old_position, obstables_to_remove = self.env_real.step_adv1(0,0,actions_sb3,probs_action_sb3)
        
        "packing the additonal info passes into the info dict required by gym standards"
        info = {'collision_status':collision_status,
